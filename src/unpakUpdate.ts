...
request("https://nexus.inpureprojects.info/ModLoader64/update/update.json", (error, response, body) => {
	if (!error && response.statusCode === 200) {
		const fbResponse = JSON.parse(body)
		console.log("Got a response: ", fbResponse);
		let version = "Nothing";
		if (fs.existsSync("./update.json")) {
			version = JSON.parse(fs.readFileSync("./update.json").toString()).version;
		}
		var options = {
			directory: "./",
		...
		if (version !== fbResponse.version) {
			...
			download("https://nexus.inpureprojects.info/ModLoader64/update/ModLoader.pak", options, function (err: any) {
				if (err) throw err;
